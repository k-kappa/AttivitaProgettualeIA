...
void loop() {
  ...
  for (int i = 0; i < (totEpoch - 1); i++) {
    ...
    float score = startEpoch(i, secondsEpoch, parameters, 50, 140);  //last 130 with full battery
  ...
  while (millis() - startTime <= secondsEpoch * 1000) {
    ...
    if (!direction) {  //Forward
      ...
      if ((previous_d1 + 40 < d1 || previous_d3 + 40 < d3) && first == 1) {  //if we measure a distance greater than the previuos one it means we avoided an object
        ...
        char buffer[100];
        sprintf(buffer, "debug dx %f < %f; sx %f < %f", previous_d1 +10, d1, previous_d3 + 10, d3);
        inviaDati(buffer);
        clearBuffer(buffer);
      }
      ...
      //track if the robot avoided an obstacle
      if (first == 0 && (d4 < 40 || d6 < 40) && wall == 0) {  //good for static obstacles
        first = 1;
      }
      /*char buffer[100];
      ...
      if ((previous_d4 + 20 < d4 || previous_d6 + 20 < d6) && first == 1) {  //if we measure a distance greater than the previuos one it means we avoided an object
        first = 0;
        score = score - 1;  //update the score in better
        char buffer[100];
        sprintf(buffer, "debug dx %f < %f; sx %f < %f", previous_d4 + 10, d4, previous_d6 + 10, d6);
        inviaDati(buffer);
        clearBuffer(buffer);
      }
      //apply the functions to identify the speed motor
