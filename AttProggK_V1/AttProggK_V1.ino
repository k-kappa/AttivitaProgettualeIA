...
void startWIFI();
float startEpoch(int numEpoch, unsigned long secondsEpoch, bool* parameters, float amplifier, int v0);
...
void loop() {
  ...
  for(int i=0;i<totEpoch;i++){
    ...
    char buffer[100];
    sprintf(buffer,"Starting epoch %d with configuration: a=%d b=%d c=%d d=%d",i,parameters[0],parameters[1],parameters[2],parameters[3]);
    inviaDati(buffer);
    clearBuffer(buffer);
    //start epoch
    float score = startEpoch(i,secondsEpoch,parameters,50,110);
    //check the score and send data to PC
    if(score<bestScore){
      char buffer[100];
      sprintf(buffer,"new best score %f with configuration: a=%d b=%d c=%d d=%d",score,parameters[0],parameters[1],parameters[2],parameters[3]);
      inviaDati(buffer);
      clearBuffer(buffer);
    }else{
      char buffer[100];
      sprintf(buffer,"score %f with configuration: a=%d b=%d c=%d d=%d",score,parameters[0],parameters[1],parameters[2],parameters[3]);
      inviaDati(buffer);
      clearBuffer(buffer);
    }
    ...
    //apply the mutation
    if(parameters[pos]==1) parameters[pos]=0;
    else parameters[pos] = 1;
