...
void loop() {
  ...
  inviaDati(buffer);
  clearBuffer(buffer);
  float score = startEpoch(0, secondsEpoch, parameters, 90, 110);
...
float startEpoch(int numEpoch, unsigned long secondsEpoch, float* parameters, float amplifier, int v0) {
  ...
  while (millis() - startTime <= secondsEpoch * 1000) {
    ...
    if (!direction) {  //Forward
      ...
      if (first == 0 && mem == 0 && (d1 < 50 || d3 < 50) && wall == 0) {  //good for static obstacles
        first = 1;
        mem = 1;
      }
      if ((previous_d1 + 2 < d1 || previous_d3 + 2 < d3) && mem == 1) {  //if we measure a distance greater than the previuos one it means we avoided an object
        first = 0;
      ...
      previous_d6 = d6;
      //track if the robot avoided an obstacle
      if (first == 0 && mem == 0 && (d4 < 50 || d6 < 50) && wall == 0) {  //good for static obstacles
        first = 1;
        mem = 1;
      }
      if ((previous_d4 + 2 < d4 || previous_d6 + 2 < d6) && mem == 1) {  //if we measure a distance greater than the previuos one it means we avoided an object
        first = 0;
        score = score - 1;  //update the score in better
      }
      //apply the functions to identify the speed motor
